...
1. 给定insert语句
   ...
   ```js
   db.banyuan.find({age:{$lt:24}})
   ```

   找到年龄小于等于20的学生，增加一个字段为important，值为true，意义在于年龄小需要重点关注，同时更新大于20的学生，important为false

   ```js
   db.banyuan.updateMany({age:{$lte:20},role:'student'},{$set:{important:'true'}})
   db.banyuan.updateMany({age:{$gt:20},role:'student'},{$set:{important:'false'}})
   ```
